...
import sifive.fpgashells.ip.xilinx.ku040mig.{KU040MIGIOClocksReset, KU040MIGIODDR, ku040mig}

case class XilinxKU040MIGParams(
  address : Seq[AddressSet]
)

class XilinxKU040MIGPads extends KU040MIGIODDR

class XilinxKU040MIGIO extends KU040MIGIODDR with KU040MIGIOClocksReset

class XilinxKU040MIGIsland(c : XilinxKU040MIGParams)(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  val ranges = AddressRange.fromSets(c.address)
  require (ranges.size == 1, "DDR range must be contiguous")
  val offset = ranges.head.base
  val depth = ranges.head.size
  val crossing = AsynchronousCrossing(8)
  require((depth<=0x40000000L),"ku040mig is a single x16 component: 1GB depth per controller")
  ...
  class Impl extends LazyModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxKU040MIGIO
    })

    //MIG black box instantiation
    val blackbox = Module(new ku040mig)
...
class XilinxKU040MIG(c : XilinxKU040MIGParams)(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxKU040MIGIsland(c))
