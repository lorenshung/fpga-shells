package sifive.fpgashells.devices.xilinx.xilinxarty200tmig
...
import sifive.fpgashells.ip.xilinx.arty200tmig.{Arty200TMIGIOClocksReset, Arty200TMIGIODDR, arty200tmig}

case class XilinxArty200TMIGParams(
  address : Seq[AddressSet]
)

class XilinxArty200TMIGPads(depth : BigInt) extends Arty200TMIGIODDR(depth) {
  def this(c : XilinxArty200TMIGParams) { this(AddressRange.fromSets(c.address).head.size) }
}

class XilinxArty200TMIGIO(depth : BigInt) extends Arty200TMIGIODDR(depth) with Arty200TMIGIOClocksReset

class XilinxArty200TMIGIsland(c : XilinxArty200TMIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  ...
  class Impl extends LazyRawModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxArty200TMIGIO(depth)
    })
    ...
    val blackbox = withClockAndReset(childClock, childReset) { Module(new arty200tmig(depth)) }
...
class XilinxArty200TMIG(c : XilinxArty200TMIGParams, crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxArty200TMIGIsland(c, crossing))
