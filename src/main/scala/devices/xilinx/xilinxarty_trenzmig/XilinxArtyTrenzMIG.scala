package sifive.fpgashells.devices.xilinx.xilinxarty_trenzmig
...
import sifive.fpgashells.ip.xilinx.arty_trenzmig.{ArtyTrenzMIGIOClocksReset, ArtyTrenzMIGIODDR, arty_trenzmig}
...
class XilinxArtyTrenzMIGIsland(c : XilinxArtyTrenzMIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  ...
  class Impl extends LazyRawModuleImp(this) {
    ...
    val blackbox = withClockAndReset(childClock, childReset) { Module(new arty_trenzmig(depth)) }
