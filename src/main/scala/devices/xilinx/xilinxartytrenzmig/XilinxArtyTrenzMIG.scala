package sifive.fpgashells.devices.xilinx.xilinxartytrenzmig
...
import sifive.fpgashells.ip.xilinx.artytrenzmig.{ArtyTrenzMIGIOClocksReset, ArtyTrenzMIGIODDR, artytrenzmig}

case class XilinxArtyTrenzMIGParams(
  address : Seq[AddressSet]
)

class XilinxArtyTrenzMIGPads(depth : BigInt) extends ArtyTrenzMIGIODDR(depth) {
  def this(c : XilinxArtyTrenzMIGParams) {
    this(AddressRange.fromSets(c.address).head.size)
  }
}

class XilinxArtyTrenzMIGIO(depth : BigInt) extends ArtyTrenzMIGIODDR(depth) with ArtyTrenzMIGIOClocksReset

class XilinxArtyTrenzMIGIsland(c : XilinxArtyTrenzMIGParams, val crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule with CrossesToOnlyOneClockDomain {
  ...
  class Impl extends LazyRawModuleImp(this) {
    val io = IO(new Bundle {
      val port = new XilinxArtyTrenzMIGIO(depth)
    })
    ...
    val blackbox = withClockAndReset(childClock, childReset) { Module(new artytrenzmig(depth)) }
    ...
    // TE0712 reference design loops the generated 200 MHz addn clock back
    // into clk_ref_i for IDELAYCTRL.
    blackbox.io.clk_ref_i     := blackbox.io.ui_addn_clk_0.asUInt.asBool
...
class XilinxArtyTrenzMIG(c : XilinxArtyTrenzMIGParams, crossing: ClockCrossingType = AsynchronousCrossing(8))(implicit p: Parameters) extends LazyModule {
  val ranges = AddressRange.fromSets(c.address)
  val depth = ranges.head.size
  ...
  val island  = LazyModule(new XilinxArtyTrenzMIGIsland(c, crossing))
